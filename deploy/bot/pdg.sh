...
pdg_fetch_release_tags(){
  local dir="${1:-$REPO_DIR}" requested="${2:-}" helper
  helper="$dir/lib/release-tags.sh"
  [[ -f "$helper" ]] || { echo "缺发布标签校验器: $helper" >&2; return 1; }
  # shellcheck source=lib/release-tags.sh
  source "$helper" || return 1
  pdg_origin_release_select "$dir" "$requested"
}
...
# A snapshot older than ConfigIO has no janitor.  Once pdg-web is stopped,
# remove only the exact root-managed staging directory so uploaded credentials
# cannot survive indefinitely after a cross-version rollback.
_pdg_clear_web_import_staging(){
  local pfx="${PDG_ROOT_PREFIX:-}" staging parent item
  pfx="${pfx%/}"
  [[ -z "$pfx" || ( "$pfx" == /* && "$pfx" != *".."* ) ]] || return 1
  staging="$pfx/var/lib/privdns-gateway/web-imports"
  [[ "$staging" == "$pfx/var/lib/privdns-gateway/web-imports" ]] || return 1
  [[ -e "$staging" || -L "$staging" ]] || return 0
  for parent in "$pfx/var" "$pfx/var/lib" "$pfx/var/lib/privdns-gateway" "$staging"; do
    [[ -d "$parent" && ! -L "$parent" ]] || return 1
  done
  [[ "$(readlink -f -- "$staging" 2>/dev/null)" == "$staging" ]] || return 1
  if [[ -z "$pfx" ]]; then
    [[ "$(stat -c '%u:%a' "$staging" 2>/dev/null)" == 0:700 ]] || return 1
  fi
  (
...
_pdg_clear_legacy_config_import_jobs(){
  local pfx="${PDG_ROOT_PREFIX:-}" jobs module
  pfx="${pfx%/}"
  [[ -z "$pfx" || ( "$pfx" == /* && "$pfx" != *".."* ) ]] || return 1
  jobs="$pfx/var/lib/privdns-gateway/web-jobs"
  module="$REPO_DIR/deploy/web/pdg-web-job.py"
  [[ -e "$jobs" || -L "$jobs" ]] || return 0
  [[ -d "$jobs" && ! -L "$jobs" && -f "$module" && ! -L "$module" ]] || return 1
  PDG_WEB_JOB_STATE_DIR="$jobs" python3 "$module" \
    legacy-cleanup-config-import
}
...
cmd_rollback(){
  ...
  if [[ ! -e "$tree/opt/pdg-web/pdgconfigio.py" ]]; then
    if systemctl is-active pdg-web >/dev/null 2>&1 \
       && ! systemctl stop pdg-web >/dev/null 2>&1; then
      _pdg_snapshot_abort "$tmp" "$q_current_before" \
        "$captured_helper" "$captured_tool" "$txn_nftexe" \
        "$current_had_live" "无法停止旧快照不支持的 ConfigIO Web service"
      return $?
    fi
    if ! _pdg_clear_legacy_config_import_jobs; then
      _pdg_snapshot_abort "$tmp" "$q_current_before" \
        "$captured_helper" "$captured_tool" "$txn_nftexe" \
        "$current_had_live" "无法停止或清理旧 Web 不认识的 config-import jobs"
      return $?
    fi
    if ! _pdg_clear_web_import_staging; then
      _pdg_snapshot_abort "$tmp" "$q_current_before" \
        "$captured_helper" "$captured_tool" "$txn_nftexe" \
        "$current_had_live" "无法安全清理旧快照不再管理的 Web import staging"
      return $?
    fi
  fi
  ...
  rmdir /opt/pdg-web/static/templates /opt/pdg-web/static /opt/pdg-web 2>/dev/null || true
...
cmd_update(){
  need_root update
  local dry_run=0 requested=""
  while (($#)); do
    case "$1" in
      --dry-run) dry_run=1; shift;;
      --target)
        (($# >= 2)) || { c_y "❌ --target 缺版本号"; return 1; }
        requested="$2"; shift 2;;
      *) c_y "❌ update 未知参数: $1"; return 1;;
    esac
  done
  ...
  if [[ "$dry_run" == 1 ]]; then
    ...
    local cur_desc tgt tgt_commit
    if ! pdg_fetch_release_tags "$REPO_DIR" "$requested"; then
    ...
    tgt="$PDG_RELEASE_TAG"; tgt_commit="$PDG_RELEASE_COMMIT"
    echo "当前: $cur_desc   最新发布: $tgt"
    echo "待更新提交(HEAD..$tgt):"
    git -C "$REPO_DIR" log --oneline "HEAD..$tgt_commit" 2>/dev/null \
      || echo "  (已是最新或无法比较)"
    return 0
  fi
  ...
  local tgt="$PDG_RELEASE_TAG" tgt_commit="$PDG_RELEASE_COMMIT" tgt_object="$PDG_RELEASE_OBJECT"
  if ! pdg_origin_release_materialize "$REPO_DIR" "$tgt" "$tgt_object"; then
    c_y "无法固定 origin 发布 tag $tgt, 中止更新。"; return 1
  fi
  if ! git -C "$REPO_DIR" reset --hard -q "$tgt_commit"; then
    c_y "git reset 到 $tgt 失败, 回滚到更新前快照…"; cmd_rollback --dir "$snap_dir" --git "$pre_sha"; return 1
  fi
  c_g "→ 已切到发布 $tgt"
  c_g "刷新代码(配置/出口/token/证书均不动)…"
  # Web 导入解析器依赖系统 PyYAML。旧安装可能没有；必须在替换 Web
  # 代码前自愈并做真实 import 硬校验，不能把缺依赖的新版本重启进生产。
  if ! apt-get update -qq \
     || ! DEBIAN_FRONTEND=noninteractive apt-get install -y -qq python3-yaml >/dev/null \
     || ! python3 -c 'import yaml' >/dev/null 2>&1; then
    c_y "python3-yaml 自愈/校验失败, 回滚到更新前快照…"
    cmd_rollback --dir "$snap_dir" --git "$pre_sha"; return 1
  fi
  # 必需文件: 任一装失败即立即回滚(拒绝新旧混部)。`! A || ! B` 在首个失败处短路。
  if   ! install -d -m755 /opt/pdg-web /opt/pdg-web/static /opt/pdg-web/static/templates \
    || ! install -d -o root -g root -m700 /var/lib/privdns-gateway/web-imports /etc/mihomo/providers \
  ...
  if ! python3 -c 'import yaml' >/dev/null 2>&1 \
     || ! python3 -m py_compile /opt/pdg-bot/*.py /opt/pdg-web/*.py 2>/dev/null; then
...
# v1.9.0: this migration is intentionally self-contained.  During an update from
# a pre-v1.9 release (including v1.6.4), the *old* updater copies the new repository and pdg script, then invokes
# the new `pdg __migrate`; it cannot know about newly-added Web modules or Python
# dependencies.  Any failure must escape __migrate so the old updater restores
...
migrate_web_config_io(){
  ...
  if python3 -c 'import yaml' >/dev/null 2>&1 \
     ...
     && cmp -s "$static_src/templates/mosdns-import.example.yaml" \
          /opt/pdg-web/static/templates/mosdns-import.example.yaml; then
    return 0
  fi
  apt-get update -qq || return 1
  DEBIAN_FRONTEND=noninteractive apt-get install -y -qq python3-yaml >/dev/null || return 1
  ...
  install -o root -g root -m644 \
    "$static_src/templates/mihomo-import.example.yaml" \
    /opt/pdg-web/static/templates/mihomo-import.example.yaml || return 1
  install -o root -g root -m644 \
...
run_all_migrations(){
  ...
  # Required even for pre-v1.9 -> v1.9.0, whose in-memory updater cannot install
  # these new files/dependencies itself.  Never downgrade this to best-effort.
  migrate_web_config_io || rc=1
