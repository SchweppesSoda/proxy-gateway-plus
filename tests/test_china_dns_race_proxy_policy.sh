...
if [[ "${install}" != *'rm -f /etc/systemd/system/{sniproxy,quic-proxy,china-dns-race-proxy,proxy-gateway-ios-profile,update-dnsdist-rules,proxy-gateway-socks,proxy-gateway-upstream-socks}.*'* ]]; then
