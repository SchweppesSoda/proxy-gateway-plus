...
EXPECTED_RELEASE_BASE="https://github.com/SchweppesSoda/proxy-gateway-plus/releases/download/v1.11.2"
...
[[ "$MOSDNS_PDG_ASSET_BASE_URL" == "$EXPECTED_RELEASE_BASE" \
  ...
  && ok "v1.11.2 Release 目录、helper 与构建产物名精确一致" \
...
# CI validates a commit before v1.11.2 assets can exist.  Its explicit guard
