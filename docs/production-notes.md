...
规则更新新增独立的新鲜度检查：geosite 与已配置规则集分别记录最近尝试、整批成功、
成功内容摘要和失败结果。48 小时未整批成功为警告，7 天为失败；部分成功仍保留失败状态。
这些记录不会因为现有服务进程健康而被忽略，也不会把首次离线 bootstrap 当成运行验证成功。
本地实现和测试范围见[规则新鲜度维护记录](maintenance/2026-09-21-rule-freshness.md)。
