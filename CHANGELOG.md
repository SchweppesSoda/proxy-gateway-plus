...
- 补清理 Web 服务名称映射中的旧定位文案，共享服务统一称为“代理插件服务”。现役未启用该服务，服务状态与配置保持不变。
- 沿用 v1.11.7 的退役检查清理与中文规则更新状态；两架构 MosDNS 产物和内核版本 pin 不变，发布下载目录绑定 v1.11.8。

## 2026-09-22 — v1.11.7

- 清理已移除功能的正常自检条目、Bot 遗留菜单/监听代码与当前功能文档；没有异常时不再显示退役说明。旧按钮只提示返回当前菜单，异常配置仍会阻止更新或恢复。
- “规则更新”改为“域名规则库 / 自定义规则集”的中文说明，展示上次完整成功距今多久；失败、部分成功、超时和长期未更新均有独立提示，不再把内部字段、时间戳、JSON 或内容哈希显示给用户。
- 健康告警使用不随时间变化的中文状态，保留原有去重与重试语义；规则收据、已有配置和更新事务格式不变。
- 共享 DNS 接管的界面名称不再误称为定位服务；普通 DNS、代理与共享 CA 能力保留。MosDNS 双架构产物与内核版本 pin 不变，发布下载目录绑定 v1.11.7。

## 2026-09-22 — v1.11.6

- 正式发布健康通知按收件人确认送达、持久化重试，以及规则来源刷新失败和新鲜度状态；详细行为与回退条件见下方维护记录。
- 退役 WLOC：旧按钮、回调和直接启用接口拒绝操作，恢复的旧配置不能重新注册插件或接管 Apple 定位域名；doctor 对残留明确报错。通用 DNS、代理和共享 CA 能力保留。
- 已有 WLOC 启用或接管残留的实例，必须先按[退役部署门](docs/maintenance/2026-09-22-wloc-retirement.md)用旧版本事务停用并验证普通业务，再替换代码；不存在残留时不启动旧功能。
- 部署验收逐项核对实际平台的必需服务，保留原已启用或运行的 Web；Web 仍为可选，不额外开启。
- MosDNS 双架构产物沿用 v1.11.5 的相同已钉死二进制与来源证明，新版本下载目录精确绑定 v1.11.6。Mihomo、MosDNS 版本与配置 schema 不变。

## 2026-09-21 — 本地维护（未发布、未部署）

- 规则更新记录尝试、整批成功时间、内容摘要与失败结果，部分成功不刷新全量成功时间；doctor 展示精确新鲜度，健康通知使用稳定状态告警。见[规则更新维护记录](docs/maintenance/2026-09-21-rule-freshness.md)。
- 健康通知按收件人区分观察、确认送达与待发状态；发送失败保留待发记录，按 10/20/40/60 分钟封顶退避，进程重启后继续。
- 部分收件人失败只重试未确认者；未确认的故障在恢复后合并为一次恢复说明。旧非空状态保守重报一次，正常基线保持安静。
- 状态使用独立非阻塞锁与同目录原子替换；新增离线回归并纳入 CI。迁移、验证限制和回滚条件见[维护记录](docs/maintenance/2026-09-21-health-notifications.md)。

## 2026-09-13 — v1.11.5

- 修复 geosite 默认关键词类型及畸形 protobuf 输入的边界检查。
- 补全维护入口的部署脚本索引；发布沿用已钉死并验证的双架构 MosDNS 产物。

## 2026-09-07 — v1.11.4

- 修复 YAML 规则集的 behavior 在 Mihomo 渲染和事务校验中被强制改成 classical 的问题；保留声明的 domain/ipcidr，避免规则集显示已添加但实际加载 0 条并走默认出口。
- 新增真实 Mihomo 规则数与请求匹配回归，覆盖精确域名、域名后缀和默认出口。

## 2026-09-07 — v1.11.3

- 修复订阅提供器节点测速误报超时：按当前策略组的 provider 归属调用专用 healthcheck 接口，普通节点与嵌套组继续使用普通代理测速接口。
- 404 等非超时接口错误显示为不可达，不再伪装成节点超时；增加真实接口路径约束的回归测试。

## 2026-09-07 — v1.11.2

- 修复纯域名 YAML 规则集无法添加的问题：精确域名与 `+.域名` 后缀均可解析，新增刷新回归，无法无损转换的通配符保留旧规则并拒绝更新。
- 规则集源下载或解析失败时显示明确提示，不再统一显示网关内部错误，也不会返回包含密钥的原始错误；无效策略目标在下载前拒绝。
- 策略组的“临时切换”改为“选择节点”，显示当前选择；新增“测速节点”，覆盖订阅提供器的运行态成员，在选择器内展示 HTTPS 延迟和失败状态。测速不会自动切换节点。
- 组内测速复用登录与 CSRF 校验，限制成员数量及并发，保持配置与当前选择不变；Release 包含两架构已校验的 MosDNS 修补产物。

## 2026-08-14 — v1.11.1

- 修复主题按钮在“跟随系统”与当前系统配色相同时看似无效的问题；改为明确的“跟随系统、
  浅色、深色”选择面板，并兼容旧版浏览器的系统主题监听与损坏的主题 Cookie。
- 系统检查不再把说明项误报为警告，也不再直接展示“都在”、`external`、`memory` 等内部原始
  文案或字段。检查结果按核心服务、加密 DNS、代理与网络、访问安全、配置与维护分组折叠，
  运行信息、连接流量、网络模式与导入预览改用专门的中文摘要和紧凑响应式排版。
- v1.11.1 Release 继续携带并校验 amd64/arm64 两份钉死的 MosDNS 修补产物；仓库默认下载
  通道与供应链测试精确绑定到本版本的不可变 Release 目录。

## 2026-08-14 — v1.11.0

- Web 控制台重做为中文优先的紧凑深浅双主题界面，七个功能区保持完整；主题默认跟随系统，
  可在顶部切换，并仅以 `pdg_theme` Cookie 保存。统一使用“代理节点、代理策略组、规则集、
  出口、代理提供器（Provider）”术语，修正 checkbox 被全局输入框样式放大的问题，并以有序
  搜索选择器管理代理策略组成员和 Provider。
- 分享链接、配置导入、API、Mihomo 渲染及 Telegram Bot 现可完整保留中文、空格、emoji 与
  常见标点名称；名称统一执行 NFC、64 Unicode 码点/256 UTF-8 字节及危险控制字符校验，
  非法名称会明确拒绝，不再静默替换或截断。名称型 Web API 新增 `~` + 无填充 Base64URL
  标识符，同时继续兼容旧 ASCII 路径；Bot 动态回调改为短摘要并按当前候选安全解析。
- 配置 schema 版本不变，也不会迁移已有 ASCII 名称。兼容性提醒：旧版 PDG 会安全拒绝含新式
  Unicode 名称的配置，不能直接用旧版程序管理这些名称。升级前及任何降级操作前请先执行
  `sudo pdg snapshot` 保留配置快照；如需降级，应先在新版中把相关名称改回旧版可接受的 ASCII。
- v1.11.0 Release 继续携带并校验 amd64/arm64 两份钉死的 MosDNS 修补产物；仓库默认下载
  通道与供应链测试精确绑定到本版本的不可变 Release 目录。

## 2026-08-13 — v1.10.1

- 移动端窄屏底栏固定展示“概览、出口、策略组、分流”四个高频入口和独立“更多”按钮；DNS、
  实时、运维以及未来新增的非高频页会从 canonical tab 标记动态收纳进可访问的更多菜单，
  无需复制页面或维护硬编码数量。桌面和平板（`>=760px`）仍直接展示全部 canonical tab。
- 移动端改用普通 navigation / `aria-current` 语义，桌面端恢复完整 tablist/tab/tabpanel 与唯一
  roving focus；更多菜单支持键盘打开、Escape/点外关闭、选择后焦点恢复，并在断点切换时把
  导航内焦点稳定转移到当前活动入口。补齐窄屏布局、动态 overflow、ARIA 状态、焦点与断点回归。
- v1.10.1 Release 继续携带并校验 amd64/arm64 两份钉死的 MosDNS 修补产物；仓库默认下载
  通道与供应链测试精确绑定到本版本的不可变 Release 目录。
...
- Web 品牌英文名统一为 `PRIVATE DNS GATEWAY`，中文名、登录标题、浏览器标题与安装应用名
  统一为清晰的“私有 DNS 网关”，不再显示含糊的“PDG 控制台”。
